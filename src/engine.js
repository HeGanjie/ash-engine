    ...
    for (let yi = minY; yi < maxY; yi++) {
      for (let xi = minX; xi < maxX; xi++) {
        ...
        if (w0 >= 0 && w1 >= 0 && w2 >= 0) {
          ...
          let r = (w0Dz0 * ca.r + w1Dz1 * cb.r + w2Dz2 * cc.r) * pDepth,
            g = (w0Dz0 * ca.g + w1Dz1 * cb.g + w2Dz2 * cc.g) * pDepth,
            b = (w0Dz0 * ca.b + w1Dz1 * cb.b + w2Dz2 * cc.b) * pDepth;
          pCheck.x = xi;
          pCheck.y = yi;
  ...
  render(scene, ctx, fov = PI / 2) {
    ...
    for (let mi = 0; mi < ndc3Ds.length; mi++) {
      let verticesPos = ndc3Ds[mi].map(v3 => {
        return new Vector3(v3.x * this.width, (1 - v3.y) * this.height, v3.z);
      });
      ...
      for (let face of faces) {
        this.fillFace(backbuffer, zBuf, face, verticesPos, verticesColor);
      }
    }
